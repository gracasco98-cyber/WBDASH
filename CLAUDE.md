...
1. ~~**Decimal invece di Float** per tutti gli importi monetari~~ — **fatto il 2026-07-30** (database era vuoto, nessun backfill necessario). Vedi `docs/tech-debt.md` sezione E.2 per come è stato implementato (conversione al confine del repository layer + estensione Prisma centralizzata per le query raw) e sezione E.1 per il debito scoperto nel farlo (repo-layer non rispettato nel dominio Amazon). **Verifica end-to-end (test Testcontainers) non ancora eseguita in un ambiente con Docker — farlo prima di sincronizzare dati reali.**
