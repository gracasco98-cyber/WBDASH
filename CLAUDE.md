...
0. ~~**Nessuno storico di migrazioni**~~ — **fatto il 2026-08-01**: `backend/prisma/migrations/20260731083435_init/` è la baseline versionata (`prisma migrate dev --name init`, generata sullo schema post-Decimal/post-multi-account). Verificata due volte: `prisma migrate status` coerente col DB dev esistente (zero drift) e `prisma migrate deploy` applicato con successo da zero su un Postgres pulito (container temporaneo, poi rimosso). Da qui in avanti ogni modifica di schema passa da `prisma migrate dev`, non più `db push`, su `develop`/`main`.
