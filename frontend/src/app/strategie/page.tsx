...
export default function StrategiePage() {
  ...
  const chooseFile = (next: File | null) => {
    ...
    const allowed = /\.(txt|md|csv|json)$/i.test(next.name);
    if (!allowed) return setError("Per ora sono supportati TXT, Markdown, CSV e JSON.");
  ...
  const analyze = async () => {
    ...
    try { const row = await api.strategies.analyze({ title: title.trim(), fileName: file.name, mimeType: file.type || "text/plain", fileSize: file.size, content }); setStrategies(rows => [row, ...rows.filter(r => r.id !== row.id)]); setSelected(row); setShowUpload(false); setTitle(""); setFile(null); setContent(""); }
