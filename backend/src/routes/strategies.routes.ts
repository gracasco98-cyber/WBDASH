...
router.post("/", async (req: Request, res: Response) => {
  const { title, fileName, mimeType, fileSize, content } = req.body ?? {};
  if (typeof title !== "string" || !title.trim() || typeof fileName !== "string" || typeof content !== "string") return res.status(400).json({ error: "Titolo e contenuto del file sono obbligatori." });
  if (content.length > MAX_TEXT) return res.status(413).json({ error: "Il documento è troppo grande (massimo 180.000 caratteri)." });
  const row = await prisma.strategy.create({ data: { title: title.trim(), fileName, mimeType: typeof mimeType === "string" ? mimeType : "text/plain", fileSize: Number.isFinite(Number(fileSize)) ? Number(fileSize) : content.length, sourceText: content, createdById: req.user?.id ?? null } });
