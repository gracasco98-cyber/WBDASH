...
export interface PpcBillingCycle {
  accountId: string;
  accountName: string;
  threshold: number;
...
/**
 * Builds one PPC threshold cycle per account in scope. A cycle starts on the
 * calendar day after the latest real advertising ServiceFee in settlements;
...
export async function findPpcBillingCycles(
  ...
  const charges = await prisma.$queryRaw<Array<{
    account_id: string;
    account_name: string;
    settlement_id: string;
    charge_date: string;
    ...
    WITH charge_events AS (
      SELECT
        ...
        t."postedDate"::date AS charge_date,
      ...
      GROUP BY t."amazonAccountId", t."settlementId", t."postedDate"::date
    ...
    SELECT DISTINCT ON (c.account_id)
      ...
      c.settlement_id,
      c.charge_date::date::text AS charge_date,
      c.amount
    FROM charge_events c
    JOIN "AmazonAccount" a ON a.id = c.account_id
    ORDER BY c.account_id, c.charge_date DESC, c.settlement_id DESC
  `);

  const spends = await prisma.$queryRaw<Array<{
    account_id: string;
    account_name: string;
    spend_date: string;
    spend: number;
    ...
    SELECT
      s."amazonAccountId" AS account_id,
      a.name AS account_name,
      s."snapshotDate"::date::text AS spend_date,
      COALESCE(SUM(s.spend), 0)::FLOAT8 AS spend
    FROM "AmazonAdSnapshot" s
    JOIN "AmazonAccount" a ON a.id = s."amazonAccountId"
    WHERE s."amazonAccountId" IN (${Prisma.join(accountIds)})
    GROUP BY s."amazonAccountId", a.name, s."snapshotDate"
    ORDER BY s."amazonAccountId", s."snapshotDate"
  `);

  const chargeByAccount = new Map(charges.map((row) => [row.account_id, row]));
  const nameByAccount = new Map(accounts.map((row) => [row.id, row.name]));
  for (const row of spends) nameByAccount.set(row.account_id, row.account_name);
  for (const row of charges) nameByAccount.set(row.account_id, row.account_name);
  const round2 = (value: number) => Math.round(value * 100) / 100;
  const round1 = (value: number) => Math.round(value * 10) / 10;

  return accountIds.map((accountId) => {
    const charge = chargeByAccount.get(accountId) ?? null;
    const eligible = spends.filter(
      (row) => row.account_id === accountId && (!charge || row.spend_date > charge.charge_date),
    );
    let cumulative = 0;
    const daily = eligible.map((row) => {
      cumulative = round2(cumulative + Number(row.spend));
      return { date: row.spend_date, spend: round2(Number(row.spend)), cumulative };
    });
    const recent = daily.slice(-7);
    const averageDailySpend7d = recent.length > 0
      ? round2(recent.reduce((sum, day) => sum + day.spend, 0) / recent.length)
      : 0;
    const accumulatedSpend = round2(cumulative);
    const remaining = round2(Math.max(0, PPC_BILLING_THRESHOLD_EUR - accumulatedSpend));
    ...
    return {
      ...
      accumulatedSpend,
      progressPct: round1(Math.min(100, (accumulatedSpend / PPC_BILLING_THRESHOLD_EUR) * 100)),
      remaining,
      status: accumulatedSpend >= PPC_BILLING_THRESHOLD_EUR ? "charge_expected" : "accumulating",
      averageDailySpend7d,
      estimatedDaysToThreshold: remaining > 0 && averageDailySpend7d > 0
        ? Math.ceil(remaining / averageDailySpend7d)
        : null,
      updatedThrough: daily.at(-1)?.date ?? null,
      lastCharge: charge
        ? {
            settlementId: charge.settlement_id,
            date: charge.charge_date,
            amount: round2(Number(charge.amount)),
          }
        : null,
      // The cumulative value covers the full cycle; the UI only needs the
      // latest week of increments, keeping the response bounded when no
      // settlement charge has been imported for a long time.
      daily: recent,
    } satisfies PpcBillingCycle;
  });
}
